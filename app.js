...
const FINE_MIN_MONDAY = new Date(2026, 4, 4);  // 5월 1주차 월요일 (5/4 Mon). 그 이전 주차 비노출 — 4/27~5/3은 표준상 4월 5주차라 제외
// 벌금 전역 시행 시작일 (이 날짜 이전 = 전원 '-', 벌금/X 없음).
// 5월 1주차 월요일 (5/4). 백엔드 FINE_ENFORCEMENT_START_와 동일해야 함.
const FINE_ENFORCEMENT_START = '2026-05-04';
  ...
  members.forEach(m => {
    ...
    for (let wi = 0; wi < chainMondays.length; wi++) {
      ...
      while (pIdx < mPays.length && mPays[pIdx].date <= wr.weekEnd) {
        running += (mPays[pIdx].amount || 0); pIdx++;
      }
